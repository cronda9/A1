...
def main():
    """Princeton Registar Class Details Search"""
    ...
    try:
        with connect(DATABASE_URL, isolation_level=None, uri=True) as connection:
            with closing(connection.cursor()) as cursor:
                ...
                if not class_info:
                    raise Exception("no class with classid "+str(class_id)+" exists")
        ...
        print('reg_details.py: '+ex, file=stderr)
        exit(1)
